...
{
    public class GetNewUsersByMonth : IAdminDashboardFeature
    {
        public static void Map(IEndpointRouteBuilder app) => app
            .MapGet($"/{nameof(GetNewUsersByMonth)}", Handle)
        ...
        private static async Task<IResult> Handle(int month, IAdminDashboardService service, ICustomLogger _logger, CancellationToken cancellationToken)
        {
            ...
            try
            {
                var result = await service.GetNewUsersByMonth(month, cancellationToken);
